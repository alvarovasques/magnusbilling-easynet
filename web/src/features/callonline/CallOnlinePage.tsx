...
import { callOnlineResource, CallOnline } from './api';
...
export function CallOnlinePage() {
  ...
  const hangup = async (id?: number) => {
    if (!id) return;
    await callOnlineResource.destroy(id);
  ...
  const columns: ColDef<CallOnline>[] = useMemo(() => [
    ...
    { field: 'callerid', headerName: 'Origem', minWidth: 140 },
    ...
    { headerName: '', width: 60, pinned: 'right', sortable: false, filter: false,
      cellRenderer: (p: any) => (
        <button title="Derrubar chamada" className="text-danger hover:text-danger-strong" onClick={() => hangup(p.data?.id)}>
          <PhoneOff size={16} />
        </button>
      ) },
  ], []);
